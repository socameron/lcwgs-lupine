...
read_theta_data <- function(file_path) {
  ...
  # Select relevant columns (window position, chromosome, and nucleotide diversity)
  theta_data <- theta_data %>% 
    select(window_position = WinCenter, chr = Chr, nucleotide_diversity = tP)
...
# Loop through each chromosome (or scaffold) and generate line plots
scaffolds <- unique(combined_theta_data$chr)

for (scaffold in scaffolds) {
  scaffold_data <- combined_theta_data %>% filter(chr == scaffold)
  
  # Create a line plot for each scaffold with nucleotide diversity along the genome
  p_nucleotide_div <- ggplot(scaffold_data, aes(x = window_position, y = nucleotide_diversity, color = population, group = population)) +
    geom_line(aes(alpha=0.5), linewidth = 0.2) +
    scale_color_manual(values = population_colors) +
    labs(title = paste("Nucleotide Diversity (θπ) - Scaffold", scaffold), 
         x = "Genomic Position", 
         y = "Nucleotide Diversity (θπ)") +
    custom_theme
  
  # Save each plot as a separate file
  ggsave(paste0("results/plots/hap2/theta/nucleotide_diversity_scaffold_", scaffold, ".png"), 
         plot = p_nucleotide_div, width = 10, height = 6)
}

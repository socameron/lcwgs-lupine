...
for (pop in populations) {
  ...
  pop_data <- ngsrelate_data %>%
    select(F) %>% # Remove rows where rab is 0
...
# Boxplot with zeroes
boxplot_1 <- ggplot(combined_data, aes(x = population, y = F, fill = population)) +
  geom_boxplot(aes(alpha=0.3)) +
  scale_fill_manual(values = population_colors) +
  labs(title = "Boxplot of Inbreeding (F) Across Populations",
       x = "Population",
       y = "F values") +
  custom_theme
ggsave("results/plots/hap2/ngsRelate/F_boxplot.png", plot = boxplot_1, width = 8, height = 6)

# Boxplot without zeroes
boxplot_2 <- ggplot(non_zero_data, aes(x = population, y = F, fill = population)) +
  geom_boxplot(aes(alpha=0.3)) +
  scale_fill_manual(values = population_colors) +
  labs(title = "Boxplot of Inbreeding (F) Across Populations",
       x = "Population",
       y = "F values") +
  custom_theme
ggsave("results/plots/hap2/ngsRelate/F_boxplot_no_zeros.png", plot = boxplot_2, width = 8, height = 6)
...
boxplot_3 <- ggplot(non_zero_data, aes(x = population, y = log10(rab), fill = population)) +
  ...
  labs(title = "Boxplot of Inbreeding (F) Across Populations",
       ...
       y = "log10 F values") +
  custom_theme
ggsave("results/plots/hap2/ngsRelate/F_log_boxplot_no_zeros.png", plot = boxplot_3, width = 8, height = 6)

# Violin Plot
violin_plot <- ggplot(non_zero_data, aes(x = population, y = F, fill = population)) +
  geom_violin(trim = FALSE) +
  scale_fill_manual(values = population_colors) +
  labs(title = "Violin Plot of Inbreeding (F) Across Populations",
       x = "Population",
       y = "F values") +
  custom_theme
ggsave("results/plots/hap2/ngsRelate/F_violin_plot_no_zeros.png", plot = violin_plot, width = 8, height = 6)

# Split Violin Plot with Boxplot Overlay
split_violin_plot <- ggplot(non_zero_data, aes(x = population, y = F, fill = population_group)) +
  ...
  labs(title = "Split Violin and Boxplot of Inbreeding (F) Across Populations (Excluding Zeros)",
       x = "Population",
       y = "F values") +
  custom_theme
ggsave("results/plots/hap2/ngsRelate/F_split_violin_boxplot_no_zeros.png", plot = split_violin_plot, width = 8, height = 6)

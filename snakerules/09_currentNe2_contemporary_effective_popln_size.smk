...
rule currentne2_estK:
  ...
  shell:
    ...
    echo "[currentNe2 estK] genome_size=24; VCF={input.vcf}" > {log}
    currentne2 -t {threads} -o {params.filename} {input.vcf} 24 >> {log} 2>&1
    """
...
rule currentne2_k:
  input:
    vcf = "results/angsd/hap2/canonical/currentNe2_input/{population}/{population}_canonical_SNPs_GT.vcf"
  output:
    filename = "results/currentNe2/hap2/{population}/{population}_k{k}_currentNe2_OUTPUT.txt"
  log:
  ...
  shell:
    ...
    set -euo pipefail
    mkdir -p results/currentNe2/hap2/{wildcards.population} results/logs/currentNe2/hap2/{wildcards.population}
    echo "[currentNe2 k] pop={wildcards.population} k={wildcards.k} genome_size=24; VCF={input.vcf}" > {log}
    currentne2 -t {threads} -k {wildcards.k} -o {output.filename} {input.vcf} 24 >> {log} 2>&1
    """
...
rule currentne2_x:
  input:
    vcf="results/angsd/hap2/canonical/currentNe2_input/{population}/{population}_canonical_SNPs_GT.vcf"
  output:
    filename="results/currentNe2/hap2/{population}/{population}_x_currentNe2_OUTPUT.txt"
  log:
  ...
  shell:
    ...
    mkdir -p results/currentNe2/hap2/{wildcards.population} results/logs/currentNe2/hap2/{wildcards.population}
    GS=$(cat {input.gs})
    echo "[currentNe2 -x] pop={wildcards.population} genome_size=24; VCF={input.vcf}" > {log}
    currentne2 -t {threads} -x -o {output.filename} {input.vcf} 24 >> {log} 2>&1
    """
